...
const offerAdd = async (req, res) => {
  try {
    ...
    const discProduct = await Products.find({ category: cats._id }).populate(
      "category"
    );
    ...
    if (!exist) {
      discProduct.forEach(async (element) => {
        ...
        await Products.findOneAndUpdate(
          { name: element.name },
          { $set: { discount: offer, discout_price: discounts } }
        );
      });
...
const removeoff = async (req, res) => {
  try {
    ...
    const productss = await Products.find({
      category: reOffer.category._id,
    }).populate("category");
    productss.forEach(async (val) => {
      await Products.findOneAndUpdate(
        { name: val.name },
        { $set: { discount: 0, discout_price: 0 } }
      );
    });
