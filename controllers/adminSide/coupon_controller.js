...
const UserCoupon = async (req, res, next) => {
  try {
    ...
    if (req.session.user) {
      ...
      res.render("Profile/userCoupons", {
        listedCategory,
...
const couponUse = async (req, res) => {
  try {
    ...
    const check = await Coupons.findOne({
      couponcode: enterCoupon,
      status: true,
    });

    if (check) {
      const useCheck = await User.findOne({
        _id: req.session.user._id,
        appliedCoupon: { $in: [check.couponcode] },
      });
      ...
      if (useCheck) {
        ...
        if (cartAmt <= check.from) {
          ...
          const deductedprice = Math.round(
            (cartData.Total_price / 100) * (100 - check.discount)
          );
          ...
          const updatedCart = await Cart.findOneAndUpdate(
            ...
            {
              $set: {
                Total_price: deductedprice,
                couponDiscount: cartAmt - deductedprice,
                discountRate: check.discount,
              },
            },
            { new: true }
          );
          ...
          await User.findOneAndUpdate(
            { _id: req.session.user._id },
            { $push: { appliedCoupon: check.couponcode } }
          );
...
const couponRemove = async (req, res) => {
  try {
    ...
    const useCart = await Cart.findOneAndUpdate(
      { userId: req.session.user._id, _id: cartid },
      {
        $set: {
          couponDiscount: 0,
          discountRate: 0,
...
const displayCoupon = async (req, res) => {
  try {
    ...
    const recentCoupon =
      couponfind.appliedCoupon[couponfind.appliedCoupon.length - 1];
