    ...
    try {
        ...
        if(Walltcheck){
            ...
            if( Walltcheck.balance >= total ){
                ...
                const caart = await Cart.findOne ({userId:userid}).populate('product.productId')

                const  addAddress =await Address.findOne({userId:userid,'addresss.status':true},{"addresss.$":1})
                    
                let truepro
                if(caart?.product){

                    const productStatus = caart.product.filter(val => val.productId.status === false||val.productId.stock<=0); 
    
                    //  Product Status
                    const falseproduct=await Cart.findOneAndUpdate({ userId: req.session.user._id }, { $pull: { product: { productId: { $in: productStatus.map(p => p.productId._id) } } } },{new:true}).populate('product.productId')
                ...
                if(cartCount>productStock){
                    ...
                    if(count==truepro.product.length){
                        ...
                        let paymentMethod = req.body.peyment
                                ...
                                await Wallet.findOneAndUpdate({userId:userid},{$set:{balance:lastBalance},$push:{transaction:{Amount:total,Status:'Debit',Source:'Wallet Purchase'}}},{upsert:true,new:true})
                            ...
                            if (saveOrder) {
                                ...
                                saveOrder.products.forEach( async(e) => {
                                    ...
                                    await Products.findOneAndUpdate({_id : e.productId} , {$set : {stock : newStock}})
...
const addMoney = async ( req , res ) => {
    try {
        ...
        const wallwtUpdate = await Wallet.findOneAndUpdate({userId:req.session.user._id},
            {$inc:{balance:recharge},$push:{transaction:{Amount:recharge,Source:'Wallet topup',Status:'Credit'}}},
