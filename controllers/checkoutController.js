        ...
        if (userProduct) {
                ...
                if (userProduct.couponDiscount>0) {
                 ...
                 he =totalPrice - userProduct.couponDiscount;
                    ...
                    const  cartData = await Cart.findOneAndUpdate({userId:req.session.user._id},{$set:{Total_price:he}},{new:true,upsert:true}).populate('product.productId')
                    const addressList = await Address.findOne({userId:req.session.user._id}) || null
                    
                    const flash = req.flash('flash')
           
                    res.render('checkout',{listedCategory,login:req.session.user,userdata,addressList,cartData,msg:flash })
        }else{
            const addressList = await Address.findOne({userId:req.session.user._id}) || null
            const flash = req.flash('flash')
            res.render('checkout',{listedCategory,login:req.session.user,userdata,cartData:0,addressList,msg:flash})
        }
