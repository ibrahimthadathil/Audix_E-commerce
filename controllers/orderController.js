...
const loadOrder = async ( req , res ) => {
    try {
        ...
        const totalCatCount = await Order.find({UserId:req.session.user._id}).countDocuments();
...
const orderplace = async ( req , res ) =>{
    try {
        ...
        let truepro
        const caart = await Cart.findOne ({userId:req.session.user._id}).populate('product.productId');
       
        const  addAddress =await Address.findOne({userId:req.session.user._id,'addresss.status':true},{"addresss.$":1})
        if(caart?.product){
            const proStatus = caart.product.filter(val => val.productId.status === false||val.productId.stock<=0);     
            //  Product Status
            const falseproduct=await Cart.findOneAndUpdate({ userId: req.session.user._id }, { $pull: { product: { productId: { $in: proStatus.map(p => p.productId._id) } } } },{new:true}).populate('product.productId')
            ...
            for(let i=0;i<truepro.product.length;i++){
                ...
                if(cartCount>productStock){
                    ...
                    if(count==truepro.product.length){
                        ...
                        const {userName,name,city,state,phone,pincode} = addAddress?.addresss?.[0] ?? {};
...
const razor = async (req, res) => {
    try {
        ...
        const cartCheck = await Cart.findOne ({userId:req.session.user._id}).populate('product.productId');
...
const cancelOrder = async ( req, res ) => {
    try {
        ...
        const cancelItem =  await Order.findOneAndUpdate({_id:cancelorder , 'products.productId' : cancelproduct},{$set:{'products.$.orderProStatus':'canceled'}},{new:true});

        let canceledProduct

      if(cancelItem){
        
             canceledProduct = cancelItem.products.find(product => product.productId.equals(cancelproduct));

            await Product.findOneAndUpdate({ _id: canceledProduct.productId },{ $inc: { stock: canceledProduct.quantity } })
        ...
        await Order.findByIdAndUpdate({_id:cancelorder},{ $set: { orderAmount: updatedOrderAmount } },{ new: true });
...
const returnProduct = async ( req , res ) => {
    try {
        ...
        const returnedItem = await Order.findOneAndUpdate({_id:orId,'products.productId':proId},{$set:{'products.$.orderProStatus':'return','products.$.orderReason':reason}},{new:true})
        if(returnedItem){
            res.send({updated:true})
            const returnedproduct = returnedItem.products.find(product=> product.productId.equals(proId))
            if(returnedproduct.orderReason=='Product Damaged'){
                await product.findOneAndUpdate({_id:returnedproduct.productId},{$inc:{damagedStock:returnedproduct.quantity}})
            }else{

                await Product.findOneAndUpdate({_id:returnedproduct.productId},{$inc:{stock:returnedproduct.quantity}})
            }
            let updatedOrderAmount= returnedItem.products.filter(product=> product.orderProStatus!='return'&&product.orderProStatus!='canceled')
            .reduce((acc,elem)=>acc+elem.price,0)

            if(returnedItem.discountRate>0){
              
    
                    updatedOrderAmount = Math.floor(( updatedOrderAmount )-( updatedOrderAmount * returnedItem.discountRate / 100 ))
                 
    
            }
            await Order.findOneAndUpdate( { _id : orId } , { $set: { orderAmount : updatedOrderAmount } })
          
            let diductPrice

            if(returnedItem.discount>0){
                diductPrice = Math.floor((returnedproduct.price)-(returnedproduct.price * returnedItem.discountRate / 100))
                console.log(diductPrice);
            }

            await Wallet.findOneAndUpdate({userId:req.session.user._id},{$inc:{balance:diductPrice},$push:{transaction:{Amount:diductPrice,Status:'Credit',Source:'Product Return'}}},{upsert:true,new:true})  // adding he return amount
