...
const addCart = async ( req , res ) => {
    try {
        ...
        if(!exist){
          ...
          const total =cartProduct.discount>0? cartProduct.discout_price * qty : cartProduct.price * qty

           const cartupdate= await Cart.findOneAndUpdate({userId:req.session.user},
            {$addToSet:{
...
const cartEdit = async (req, res) => {
    try {
      ...
      const newval =product.discount> 0 ? product.discout_price * req.body.quantity : product.price * req.body.quantity
